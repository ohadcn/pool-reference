...
class SqlitePoolStore(AbstractPoolStore):
    ...
    async def get_points_by_date(self, launcher_id: bytes32, count: int) -> List[Tuple[uint64, uint64]]:
        cursor = await self.connection.execute(
            "SELECT strftime('%Y.%m.%d',datetime(timestamp, 'unixepoch')) as date, SUM(difficulty) as points from partial WHERE launcher_id=? AND strftime('%s', 'now','-1 day')>timestamp GROUP BY strftime('%Y.%m.%d',datetime(timestamp, 'unixepoch')) LIMIT ?",
            (launcher_id.hex(), count),
        )
        rows = await cursor.fetchall()
